...
static void sinkCandidatesImpl(BasicBlock *BBEnd,
                               ArrayRef<BasicBlock *> IncomingBBs,
                               ArrayRef<Instruction *> Insts,
                               bool allowOtherInstsToHaveNonDbgUsers) {
...
// phi(...)
static bool
herusticGetPhiGEPCandidate(SmallVectorImpl<Instruction *> &ResultsSinkCandidate, ArrayRef<BasicBlock*> Preds,
  ...
  SmallVector<SmallVector<Instruction*, 0>, 0> SinkCandidates;
  for (auto &[U, IncomingValues] : PHIOperands) {
    ...
    SmallVector<Instruction *, 0> Candidates;
    ...
    for (auto IncomingValue : IncomingValues) {
    	if (auto GEP = dyn_cast<GetElementPtrInst>(IncomingValue); GEP && sameBase(GEP)) {
...
static bool herusticPhiSinker(BasicBlock *BB, DomTreeUpdater *DTU) {
  SmallVector<Instruction*, 0> SinkCandidates;
  // Collect all the predecessors.
  SmallVector<BasicBlock *, 0> Preds;
  for (auto *PredBB : predecessors(BB))
  ...
  bool Changed = false;
  if (!herusticGetPhiGEPCandidate(SinkCandidates, Preds, Changed, BB, DTU))
    return Changed;
  
  TinyPtrVector<Instruction*> BaseCandidatesToSkip;
  do {
    LLVM_DEBUG( [&]() {
      dbgs() << "SINK: Sink: " << *SinkCandidates[0] << "\n";
      if (SinkCandidates.size() != 1) {
        dbgs() << "SINK: Sink: other insts of" << *SinkCandidates[0] << ":\n";
        for (size_t i = 1; i < SinkCandidates.size(); i++) {
          dbgs() << " " << i << ") " << *SinkCandidates[i] << "\n";
        }
        dbgs() << "\n";
      }
    });
  ...
  } while ( herusticGetPhiGEPCandidate(SinkCandidates, Preds, Changed, BB, DTU, BaseCandidatesToSkip) ) ;
  NumSinkCommonInstrs += BaseCandidatesToSkip.size();
  ++NumSinkCommonCode;
  return true;
}
